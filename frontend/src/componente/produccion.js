...
export default class produccion extends Component {
    ...
    componentDidMount() {
        //console.log('datosProduccion', this.readCSV() )
        this.dibujarSVG();
    }

    readCSV() {
        let datosProduccion = [];
        d3.csv('./1.csv', function (data, csvdata) {
            datosProduccion.push({
                eco2: data.eco2,
                humedad: data.humedad,
                temperatura: [data.temperatura, data.time_index],
                tvoc: data.tvoc,
    ...
    dibujarSVG() {
        ...
        console.log('length', data[0].temperatura)
        ...
        //创建一个线段生成器
        var linePath = d3.line()                                    //创建一个线段生成器
            .curve(d3.curveBasis)                           //使用basis插值模式
        ...
        //坐标轴x轴
        var xAxis = d3.axisBottom()               //创建一个新坐标轴
            .scale(xScale)              //设定x坐标轴的比例尺
            .ticks(6)                   //设定x坐标轴的分隔数
            .tickFormat(d3.format("d")) //刻度的数组用字符串表示
        //设定x坐标轴的方向
        //坐标轴y轴
        var yAxis = d3.axisLeft()               //创建一个新坐标
